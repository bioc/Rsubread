...
void
generate_filenames(char *dataset, char *dir){
	...
	strcpy(simplified_SAM_file, dataset);
	strcat(simplified_SAM_file,"_simplified.txt");

	strcpy(sorted_simplified_SAM_file, dataset);
	strcat(sorted_simplified_SAM_file,"_simplified_sorted.txt");

	strcpy(annotation_exon_file, dir);
	strcat(annotation_exon_file,"exon_GC.txt");
	printf("%s\n", annotation_exon_file);
	
	
	strcpy(annotation_ir_file, dir);
	strcat(annotation_ir_file,"binned_integenic_region_GC.txt");
	printf("%s\n", annotation_ir_file);
	

	strcpy(mapping_result_exon, dataset);
	strcat(mapping_result_exon,"_mapping_exon_GC.txt");
	
	strcpy(mapping_result_ir, dataset);
...
sort_reads(void){
	...
	for (i=0; i<24; i++){
		...
		// now we have a list of int32_t specifiying the read positions
		// we need to sort it in order and export it.
		// quick sort()
...
void
detectionCall(char **dataset, char **dir){
	generate_filenames(*dataset, *dir);
