...
#define SB_FETCH_ONEREAD(a)   { int init_have = (a) -> input_binary_stream_write_ptr - (a) -> input_binary_stream_read_ptr; \
    if(init_have <4){\
    if((a)-> is_eof ) (a)->is_bam_broken = 1;else{ int test_rlen_2 = SamBam_fetch_next_chunk(a); if(test_rlen_2 +init_have <4){(a)->is_bam_broken = 1;}}}\
    if(!(a)->is_bam_broken){ \
