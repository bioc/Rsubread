...
void show_progress(global_context_t * global_context, thread_context_t * thread_context, unsigned int current_read_no, int task)
{
	...
	if(current_read_no>1000 && !progress_report_callback)
		print_in_box(81,0,0, "%.1f%%%% completed, used %.1f mins, total=%.1fk %s, speed=%.1fk/s      \r", finished_rate*100, (miltime() - global_context -> start_time)/60,guessed_all_reads*1./1000, global_context -> input_reads.is_paired_end_reads?"frags":"reads", reads_per_second/1000, reads_finished_in_this_chunk);
