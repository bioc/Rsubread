...
void final_matchingness_scoring(int use_base_scoring, const char read_str[], const char quality_str[], unsigned char is_negative_strand, int read_len, gene_vote_t * vote, unsigned short current_masks, gene_allvote_t * allvote, unsigned int qid, gene_value_index_t * my_array_index, int space_type, int indel_tolerance, int quality_scale, int total_subreads);
...
void show_cigar(char * info, int len, int is_reversed_map, char * buf, int max_indel, int total_subreads, char * read, int * pos_offset, int * adjust_rl, int * is_safegurded);
...
void compress_cigar(char * cigar,  int read_len, char *read, int * position_offset, int * adjust_rl, int * is_safegurded);
...
int test_big_margin( gene_allvote_t *allvotes, int qid);
