...
cellCounts <- function( index, sample, input.mode = "BCL", cell.barcode = NULL, nsubreads = 15,
  ...
  if(input.mode=="FASTQ" || input.mode == "FASTQ-dir"){
    ...
    if(is.visiumHD.data)stop("BCL mode doesn't support Visium HD. Please use the FASTQ file input.")
  ...
  if(detectJunctions){
    if(F){ # "detectJunctions" means detect cell-level junctions.
    ...
    }else cat("\nWARNING ===== SAMPLE-LEVEL JUNCTIONS NOT REPORTED ======\n\n")
